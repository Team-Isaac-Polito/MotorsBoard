...
#define MOTOR_TIMEOUT 1000  // Timeout in milliseconds to stop motors if no commands are received

class MotorI2C {
  public:
    MotorI2C(TwoWire* wire, SmartMotor* leftMotor, SmartMotor* rightMotor);
  ...
  private:
    ...
    static MotorI2C* instance;            // Static instance for I2C callbacks
    static void receiveDataStatic(int numBytes) { if(instance) instance->receiveData(numBytes); }
    static void sendTelemetryStatic() { if(instance) instance->sendTelemetry(); }
};
